...
- **Finding-identifier sync**: The `F<n>`/`S<n>` finding identifier convention (F for Critical/Important findings, S for Suggestions, numbered globally within each prefix) and the `<findings>` handoff parameter are synchronized across: `commands/pr-review.md` (Step 3 Skill instruction, Step 4 comment format, Step 5 subagent prompt, Step 9 handoff), `commands/pr-review-fix.md` (argument-hint, Step 1 examples/parsing, Step 3 extraction), and `README.md` (command table, phase description, and quick reference). The five "plain words" reminder lines in `pr-review.md` (Steps 4, 6, 8) must also use the updated F/S-permitting wording. Any change to the identifier scheme or parameter name must be applied to all locations.
...
- **Task-tracking instruction sync**: The instruction `IMPORTANT: After reading the development plan phases, add each phase as a task to your todo list so you do not skip any phases.` is appended to the Skill invocation context block in commands that delegate to `/feature-dev:feature-dev`: `commands/issue-implement.md` (Step 4), `commands/pr-review-fix.md` (Step 4), and `commands/pr-ci-fix.md` (Step 4). Any change to the instruction wording must be applied to all three locations. Note: per-command task tracking PRs will rewrite this instruction to use explicit `TaskCreate`/`TaskUpdate` tool references; the sync bullet should be updated at that time to reflect the new wording.
...
Commands should use `TaskCreate` and `TaskUpdate` to establish visible progress checkpoints. Commands that use these tools must include `TaskCreate, TaskUpdate` in their `allowed-tools` frontmatter (these are separate from the `Task` tool used for subagent invocation).

- **Create all tasks upfront** after input parsing, before work begins. All start as `pending`. Task creation itself is not a task.
- **Subject line**: Imperative form ("Explore the codebase"). Set `activeForm` for spinner text ("Exploring the codebase").
- **State transitions**: `in_progress` immediately before starting; `completed` after finishing, before next task. On error, leave `in_progress` -- don't artificially complete.
- **Scope**: Target 3-5 tasks per command. Fold preparatory steps (checkout, reading context) into the first substantive task.
- **Skill delegations**: One task wraps the entire Skill call -- `in_progress` before, `completed` after. Don't track the skill's internal phases.
- **User interaction loops**: One task covers the full draft/modify/approve cycle.
