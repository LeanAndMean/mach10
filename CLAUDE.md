...
- **Task-tracking instruction sync**: The instruction `IMPORTANT: After reading the development plan phases, add each phase as a task to your todo list so you do not skip any phases.` is included in the Skill invocation context block in commands that delegate to `/feature-dev:feature-dev`: `commands/issue-implement.md` (Step 4) and `commands/pr-ci-fix.md` (Step 4). Any change to the instruction wording must be applied to both locations. Note: all four natively tracked commands (`pr-review`, `pr-review-fix`, `pr-pre-merge`, `doc-review`) use natural-language task-tracking phrasing in their prose -- no tool names in command bodies. `pr-review-fix` also uses natural-language phrasing in its Step 3 Skill delegation blockquote (with `"Phase N:"` convention). The remaining per-command task tracking PRs (see #81) should follow the same natural-language convention when adding tracking to the two remaining feature-dev-delegating commands.
...
- **Post-delegation re-anchoring**: When a tracked step delegates to a Skill (via the Skill tool), include re-anchoring prose between the delegation block and the completion marker. Long-running Skill calls (especially `/feature-dev:feature-dev`) consume significant context, and the model can lose track of post-delegation instructions. Add a bridging sentence that re-establishes the command's position in the workflow before the "Mark Step N complete." line. This applies to Skill delegations only -- Task subagent calls do not consume the outer model's context and do not need re-anchoring. See `pr-review-fix.md` Step 3 for an example.
