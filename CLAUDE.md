...
- **Plan marker sync**: The `<!-- mach10-plan -->` HTML marker is used to reliably locate implementation plan comments. The marker is emitted by `commands/issue-plan.md` (Step 6) and `commands/issue-plan-review.md` (revised plan posting). It is consumed by `commands/issue-implement.md` (Step 3), `commands/issue-plan-review.md` (Step 2), `agents/feature-completeness-checker.md` (Step 1), and `commands/push.md` (Step 5, fresh-session plan lookup). Any change to the marker convention must be applied to all locations.
