...
- Use `[context]` as the standard name for any optional free-text argument in `argument-hint`. Do not use command-specific labels like `[scope]`, `[aspects]`, or `[optional-description]`. For commands that delegate to a Skill, forward context using `> **User context:** <context>` as the last item in the Skill invocation block (include only when context was provided). For commands that launch exploration/review agents, include context in each agent's prompt to focus their work. Context represents user guidance Claude should actively honor, not informational metadata: parse it for both descriptive content (folded into drafts, summaries, or agent focus) AND directives (acted on in the appropriate downstream step -- e.g., skip a section, preselect a default, set a label). When forwarding context, include enough framing that the recipient (Skill or agent) treats it as guidance to act on, not passive information. Re-asking the user something they already stated in context is friction without safety benefit: explicit skip directives ("skip release", "skip version bump") may bypass the routing question that would have asked whether to perform the optional action, since the user has already declined and no action results. However, confirmation gates that authorize Claude to *execute* a substantive action (drafting and creating a release, modifying an issue body, deleting a branch, merging a PR) remain authoritative regardless of context -- a directive like "tag as v2.0.0" sets the recommended default and pre-fills the draft, but Claude still gets explicit approval before the action runs.
...
- Commands that need codebase exploration or architecture analysis can delegate to subagents from dependent plugins (e.g., `feature-dev:code-explorer`, `feature-dev:code-architect`)
...
- **Contributing-guide lookup sync**: The lookup pattern (`CONTRIBUTING.md` → `DEVELOPMENT.md` → `.github/CONTRIBUTING.md`, first-match-stop) is duplicated in three command files: `commands/issue-plan.md` (Step 2a), `commands/issue-plan-review.md` (Step 2a), and `commands/pr-pre-merge.md` (contributing guide lookup). `README.md` (the "Customizing with CONTRIBUTING.md" section) also documents this feature. Any change to the lookup logic must be applied to all four locations.
- **Plan marker sync**: The `<!-- mach10-plan -->` HTML marker is used to reliably locate implementation plan comments. The marker is emitted by `commands/issue-plan.md` (Step 6) and `commands/issue-plan-review.md` (revised plan posting). It is consumed by `commands/issue-implement.md` (Step 3), `commands/issue-plan-review.md` (Step 1), `agents/feature-completeness-checker.md` (Step 1), and `commands/push.md` (Step 5, fresh-session plan lookup). Any change to the marker convention must be applied to all locations.
...
- **Finding-identifier sync**: The `F<n>`/`S<n>` finding identifier convention (F for Critical/Important findings, S for Suggestions, numbered globally within each prefix) and the `<findings>` handoff parameter are synchronized across: `commands/pr-review.md` (Step 2 Skill instruction, Step 3 comment format, Step 4 subagent prompt, Step 8 handoff), `commands/pr-review-fix.md` (argument-hint, Step 0 examples/parsing, Step 2 extraction), `commands/issue-plan-review.md` (Step 3 identifier assignment, Step 4 subagent prompt references, Step 5 filter conditions for plan revision and marker emission), and `README.md` (command table, phase description, and quick reference). The five "plain words" reminder lines in `pr-review.md` (Steps 3, 5, 7) and the two "plain words" reminder lines in `issue-plan-review.md` (Step 5, in the Proceed and Cancel decision-comment formats) must also use the updated F/S-permitting wording. Any change to the identifier scheme or parameter name must be applied to all locations. Note: `issue-plan-review.md` does not consume a `<findings>` handoff parameter -- the identifier scheme is internal to the command's own Step 4-to-Step 5 dataflow, not passed to a downstream command.
- **`--comments` two-call sync**: The parenthetical explaining why two `gh` CLI calls are needed (`--comments` returns only comments and silently drops the title and body, so both calls are required) is duplicated across six command files: `commands/pr-create.md` (Step 1), `commands/issue-plan.md` (Step 1), `commands/issue-implement.md` (Step 3), `commands/issue-assessment.md` (Step 2), `commands/issue-plan-review.md` (Step 1), and `commands/doc-review.md` (Step 2). Any change to the parenthetical wording must be applied to all six locations. Note: `doc-review.md` targets PRs (`gh pr view`) while the other five target issues (`gh issue view`); only the parenthetical wording is synchronized.
...
- **No-background-agent sync**: The background-agent prohibition is duplicated across six command files at ten agent-spawn sites. The multi-agent wording is: "Do not run these subagents in the background. For parallel execution, launch them in a single message instead." The single-agent wording (used at two sites) is: "Do not run this subagent in the background." The ten sites are: `commands/issue-assessment.md` (Step 3), `commands/issue-plan.md` (Steps 2b and 4), `commands/issue-plan-review.md` (Steps 2b and 4), `commands/doc-review.md` (Steps 3, 4, and 7), `commands/test-audit.md` (Step 3), and `commands/pr-review.md` (Step 4). All sites use the multi-agent wording except `commands/pr-review.md` Step 4 and `commands/issue-plan-review.md` Step 4, which use the single-agent variant because they spawn a single sequential agent rather than a parallel fan-out. Any change to the instruction wording must be applied to all ten sites. Note: `commands/pr-review.md` Step 2 (the first `Always include this instruction in the Skill invocation` directive) contains a separate Skill pass-through instruction that tells the downstream `pr-review-toolkit` Skill not to use background agents -- that is a different mechanism (Skill delegation vs. direct subagent call) and is not part of this sync.
...
Commands should use task-tracking tools to establish visible progress checkpoints. When a command adopts task tracking, include `TaskCreate` and `TaskUpdate` in its `allowed-tools` frontmatter (these are separate from the tool used for subagent invocation). Command bodies use natural-language intent rather than explicit tool names: "create a task" maps to `TaskCreate`, and "mark a task in progress" or "mark a task complete" maps to `TaskUpdate`. This section is the single source of truth for the tool-name mapping.
...
- Agents are invoked as subagents -- they do not have `allowed-tools` or `argument-hint` fields
